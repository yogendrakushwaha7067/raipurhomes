...
class _PaypalWidgetState extends State<PaypalWidget> {
        ...
        body: SafeArea(
          child: WebView(
            ...
            onWebViewCreated: (WebViewController controller) {
              ...
              // log("payyy ${"${AppSettings.baseUrl}${Api.paypal}?package_id=${widget.pacakge.id}&amount=${widget.pacakge.price.toString()}"}");
              Dio()
                  .get(
                      "https://testbrokerhub.wrteam.in/api/paypal?package_id=${widget.pacakge.id}&amount=${widget.pacakge.price.toString()}",
                      options: Options(headers: {
                        "Authorization": "Bearer ${HiveUtils.getJWT()}"
                      }))
                  .then((value) {
                log("####$value");
              });
  ...
  _getResponse(NavigationRequest request, Function(dynamic msg)? onSuccess,
    ...
    if (payerID != null) {
      ...
      log(("$uri:${Uri.parse(AppSettings.baseUrl).host}").toString(),
          name: "@compare");
